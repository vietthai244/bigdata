<template lang="pug">
    .withdrawal-record
        ...
        style='max-height: 500px; overflow: auto'
            ...
            q-tr.cursor-pointer(slot='body', slot-scope='props', :props='props')
                q-td(v-for='col in props.cols', :key='col.name', :props='props')
                    span(:style="{ color:props.row.amount >= props.row.actual_amount ? '#00C800': 'red' }")
    ...
    export default {
        ...
        methods: {
            ...
            queryHandler(params) {
                ...
                axios.DrawSelect(params).then(res => {
                    ...
                    if (res.status === 200) {
                        ...
                        let actual_amount_arr = res.data.map(item => {
                            return parseInt(item.actual_amount);
                        });
                        this.chartOptions.series[1].data = actual_amount_arr;                     //实际出款金额
                        this.total_actual_amount = actual_amount_arr.reduce((prev, cur) => {      //算计总出款金额
                            return (prev && cur) ? prev + cur : 0;
                        });
